!function(e,R){"object"==typeof exports&&"object"==typeof module?module.exports=R():"function"==typeof define&&define.amd?define("dscc",[],R):"object"==typeof exports?exports.dscc=R():e.dscc=R()}(window,function(){return t={},n.m=C={"./src/index.ts":
  ...
  */
  ...
  // Verificar si D3.js está disponible
if (typeof d3 === "undefined") {
  console.error("Error: D3.js no está definido. Asegúrate de que Looker Studio lo está cargando correctamente desde el bucket.");
}

// Verificar si dscc está definido antes de llamar a la función
if (typeof dscc !== "undefined") {
  dscc.subscribeToData(drawViz, { transform: dscc.tableTransform });
} else {
  console.error("Error: dscc no está definido. Asegúrate de que Looker Studio está cargando correctamente.");
}

function drawViz(data) {
  console.log("Data recibida de Looker Studio:", data);

  if (!data || !data.tables || !data.tables.DEFAULT || !Array.isArray(data.tables.DEFAULT.rows)) {
    console.error("Error: Datos no disponibles o mal estructurados", data);
    return;
  }

  // Obtener o crear el contenedor raíz
  let container = document.getElementById('root');
  if (!container) {
    console.warn("No se encontró 'root', creando uno automáticamente.");
  ...
  const wordsData = data.tables.DEFAULT.rows.map(row => ({
    text: row[0],
    size: row[1] || 20 // Ajuste para asegurar que el tamaño de fuente sea válido
  }));

  if (wordsData.length === 0) {
    console.error("Error: No hay palabras en los datos recibidos");
    container.innerHTML = '<p style="text-align:center;color:red;">No hay datos disponibles</p>';
    return;
  }

  // Verificar si D3.js está disponible antes de usarlo
  if (typeof d3 === "undefined") {
    console.error("Error crítico: D3.js sigue sin estar definido en este punto");
    return;
  }
  ...
  const layout = d3.layout.cloud()
    .size([dscc.getWidth(), dscc.getHeight()])
    .words(wordsData)
    .padding(5)
    ...
    .on("end", words => {
      svg.append("g")
        .attr("transform", `translate(${dscc.getWidth() / 2}, ${dscc.getHeight() / 2})`)
        .selectAll("text")
        .data(words)
        .enter().append("text")
